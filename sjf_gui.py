...
class SJF_GUI:
    ...
    def create_widgets(self):
        ...
        self.processes_frame.pack()

        self.process_label = tk.Label(self.processes_frame, text="Enter number of processes:")
        self.process_label.grid(row=0, column=0)
        self.process_entry = tk.Entry(self.processes_frame)
        self.process_entry.grid(row=0, column=1)
        self.submit_button = tk.Button(self.processes_frame, text="Submit", command=self.get_processes)
        self.submit_button.grid(row=0, column=2)

        self.gantt_chart_label = tk.Label(self.root, text="Gantt Chart")
        self.gantt_chart_label.pack()
    ...
    def create_process_entries(self, num_processes):
        ...
        self.submit_button = tk.Button(self.root, text="Submit", command=self.schedule_processes)
        self.submit_button.pack()

        for i in range(num_processes):
            frame = tk.Frame(self.root)
            frame.pack()
            process_label = tk.Label(frame, text=f"Process {i+1}:")
            process_label.grid(row=0, column=0)
            arrival_label = tk.Label(frame, text="Arrival Time:")
            arrival_label.grid(row=0, column=1)
            burst_label = tk.Label(frame, text="Burst Time:")
            burst_label.grid(row=0, column=3)
            ...
            burst_entry.grid(row=i+1, column=3)
